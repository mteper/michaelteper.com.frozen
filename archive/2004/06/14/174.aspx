...
<script type="text/javascript">
 ...
 var postTitle = ".NET String.Replace() performance uncovered";
 var postURL = "http://www.michaelteper.com:443/archive/2004/06/14/174.aspx";
...
<script type="text/javascript" src="http://www.cocomment.com/js/cocomment.js">
	...
	<body>
		<form name="Form1" method="post" action="174.aspx" id="Form1">
...
posts - 105, comments - 55, trackbacks - 1</div>
	<div id="switcher">
	    <a id="switchlink" href="174.aspx#" title="Click here to change the content width" style="display:none;"><span></span></a>
	</div>
</div>
...
<div class="previousNext">
	<a id="ctl06_PreviousNext_PrevLink" title="previous post" href="173.aspx">&lt;&lt; Unexpected string performance in .Net 1.1</a>
	...
	<a id="ctl06_PreviousNext_NextLink" title="next post" href="../23/175.aspx">Wesner on the evolution of code editors &gt;&gt;</a>
</div>


<div class="entry">
	<h4><a id="ctl06_TitleUrl" title="Title of this entry." href="174.aspx">.NET String.Replace() performance uncovered</a></h4>
	<P>In a <A href="http://michaelteper.com/archive/2004/06/14/173.aspx">previous entry</A> I ran into some unexpected results with the performance of String.Replace() in .Net Framework 1.1. Upon further digging, it turns out that Replace is only fast when the input string does not contain the string (or character) that is intended for replacement. When the string does contain it, the performance of CleanString class drops, and, as expected, the character array exhibits better perf. So here's an updated bit of code that behaves no worse than original when there is nothing to replace and better than original when there is cleaning to do:</P><PRE class=csharpcode>        <SPAN class=kwrd>public</SPAN> <SPAN class=kwrd>static</SPAN> <SPAN class=kwrd>string</SPAN> CleanString(<SPAN class=kwrd>string</SPAN> source)
        {
            <SPAN class=rem>// new result string is only allocated if it is needed</SPAN>
            <SPAN class=rem>// otherwise, the method only does a simple scan</SPAN>
            <SPAN class=kwrd>char</SPAN>[] result = <SPAN class=kwrd>null</SPAN>;

            <SPAN class=kwrd>int</SPAN> resultIndex = 0;
            <SPAN class=kwrd>foreach</SPAN> (<SPAN class=kwrd>char</SPAN> c <SPAN class=kwrd>in</SPAN> source)
            {
                <SPAN class=kwrd>if</SPAN> (IsBadCharacter(c))
                {
                    <SPAN class=kwrd>if</SPAN> (<SPAN class=kwrd>null</SPAN> == result)
                    {
                        result = <SPAN class=kwrd>new</SPAN> <SPAN class=kwrd>string</SPAN>(<SPAN class=str>' '</SPAN>, source.Length).ToCharArray();
                    }
                }
            ...
            <SPAN class=kwrd>return</SPAN> (<SPAN class=kwrd>null</SPAN> == result) ? source : <SPAN class=kwrd>new</SPAN> <SPAN class=kwrd>string</SPAN>(result).Trim();
        }

        <SPAN class=kwrd>private</SPAN> <SPAN class=kwrd>static</SPAN> <SPAN class=kwrd>bool</SPAN> IsBadCharacter(<SPAN class=kwrd>char</SPAN> c)
        {
            <SPAN class=kwrd>return</SPAN> <SPAN class=kwrd>char</SPAN>.IsDigit(c);
        }
</PRE>
   ...
   <rdf:RDF 
      ...
      <rdf:Description
         rdf:about="http://www.michaelteper.com:443/archive/2004/06/14/174.aspx"
         dc:identifier="http://www.michaelteper.com:443/archive/2004/06/14/174.aspx"
         dc:title=".NET String.Replace() performance uncovered"
         trackback:ping="/services/trackbacks/174.aspx" 
      />
   </rdf:RDF>
