...
<script type="text/javascript">
 ...
 var postTitle = "CompositeDataBoundControl explained";
 var postURL = "http://www.michaelteper.com:443/archive/2006/02/14/5922.aspx";
...
<script type="text/javascript" src="http://www.cocomment.com/js/cocomment.js">
	...
	<body>
		<form name="Form1" method="post" action="5922.aspx" id="Form1">
...
posts - 105, comments - 55, trackbacks - 1</div>
	<div id="switcher">
	    <a id="switchlink" href="5922.aspx#" title="Click here to change the content width" style="display:none;"><span></span></a>
	</div>
</div>
...
<div class="previousNext">
	<a id="ctl06_PreviousNext_PrevLink" title="previous post" href="../13/5907.aspx">&lt;&lt; Best blond joke ever!</a>
	...
	<a id="ctl06_PreviousNext_NextLink" title="next post" href="../15/5932.aspx">Google Sitemaps robots.txt tool fixed! &gt;&gt;</a>
</div>


<div class="entry">
	<h4><a id="ctl06_TitleUrl" title="Title of this entry." href="5922.aspx">CompositeDataBoundControl explained</a></h4>
	<p>The .Net framework version 2.0 includes some new base classes that make creating custom ASP.Net controls much easier. Among them is the <font face="Courier New">CompositeDataBoundControl</font> class intended to aid in the development of custom databound controls that are made up of other Controls.</p>
<p>Under most circumstances, all the control author has to do is override the <font face="Courier New">CreateChildControls</font> method. The exact method signature is:</p>
<p><font face="Courier New">protected abstract int CreateChildControls (<br /> IEnumerable dataSource,<br /> bool dataBinding<br />)</font></p>
<p>Note the int return value -- this should be the number of data items in the data source that your control acted on when processing the data source. This should become clear in a minute.</p>
<p>The <font face="Courier New">CreateChildControls</font> method will be called under two distinct scenarios: when your control is databound (and a <font face="Courier New">dataSource</font> is supplied) and on postback (when a data source is NOT available). Under this second scenario, the <font face="Courier New">dataSource</font> value will be an array of null's of the length you returned when the control was first databound. It is up to you to recreate enough of the control structure in the second scenario for the event code to work.</p>
<p>Here's a rough example. Say you have a custom control that renders a Label and a Button for each data item. The <font face="Courier New">dataSource</font> contained 5 items and so your control rendered 5 Label+Button combinations and the <font face="Courier New">CreateChildControls</font> method returned 5. In order for the button click event to work on postback, the Buttons have to be recreated on postback, but if all that is needed are Buttons, it would be inefficient to re-bind or even to employ ViewState. What happens is that on PostBack the <font face="Courier New">CreateChildControls</font> is called with an array of 5 nulls and <font face="Courier New">dataBinding</font> set to false. All you have to do in your code is detect that condition and recreate the 5 buttons. Voila, events work!</p>
<p>Hope that helps someone.</p>
   ...
   <rdf:RDF 
      ...
      <rdf:Description
         rdf:about="http://www.michaelteper.com:443/archive/2006/02/14/5922.aspx"
         dc:identifier="http://www.michaelteper.com:443/archive/2006/02/14/5922.aspx"
         dc:title="CompositeDataBoundControl explained"
         trackback:ping="/services/trackbacks/5922.aspx" 
      />
   </rdf:RDF>
...
<div id="comments">
	...
	<p></p>
			...
			<div class="target comment">
				<h4>
					<a id="ctl08_CommentList_ctl00_EditCommentImgLink"></a><a title="permalink: re: CompositeDataBoundControl explained" href="5922.aspx#20878">#</a>&nbsp;<a name="20878"></a>re: CompositeDataBoundControl explained
				</h4>
				<div class="info">Left by <a id="ctl08_CommentList_ctl00_NameLink" title="http://fekih.mehdi@gmail.com/" href="http://fekih.mehdi@gmail.com/" target="_blank">Mehdi</a> at 4/13/2006 5:30 AM</div>
				<div class="gravatar"><img id="ctl08_CommentList_ctl00_GravatarImg" src="../../../../images/services/IdenticonHandler.ashx@code=56118310" alt="Gravatar" width="50" border="0" /></div>
				<div class="post">
					<div>
						Hi and thanks for this blog<br /><br />I wanted to create a control composed of an Image and a DropDownList and I succeded to do that by extedning DataBoundControl  Class and rendreing the child controls in render method, I tried many times to use CompositeDataboundControl but I failed because there is no documentation about that, <br /><br />If we can do the same thing with DataBoundControl what are the apports of CompositeDataBoundControl ??<br /><br />Best Regards <br />Fekih Mehdi<br />Sun Certified Programmer For The Java 2 Platform 1.4
					</div>
